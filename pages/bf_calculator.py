...
with col1:
    weight = st.number_input("Starting weight (lbs)", min_value=80.0, max_value=500.0, value=187.0, step=1.0)
    start_bf = st.number_input("Starting BF%", min_value=1.0, max_value=60.0, value=26.9, step=0.1, format="%.1f")

fat_mass_start = weight * start_bf / 100
lean_mass_start = weight - fat_mass_start

with col2:
    fat_lost = st.number_input("Fat lost (lbs)", min_value=0.0, max_value=fat_mass_start, value=14.5, step=0.5)
    muscle_gained = st.number_input("Muscle gained (lbs)", min_value=0.0, max_value=100.0, value=5.0, step=0.5)
    tdee = st.number_input("Maintenance calories (TDEE)", min_value=1000, max_value=6000, value=2200, step=50)
...
c3.metric("Weight change", f"-{fat_lost:.1f}f / +{muscle_gained:.1f}m", f"{-fat_lost + muscle_gained:+.1f} lbs net")
...
w1, w2 = st.columns(2)
w1.metric("Fat loss per week", f"{fat_per_week:.2f} lbs/wk", f"{fat_per_week * CAL_PER_LB_FAT:.0f} cal deficit/wk")
...
d2.metric("Daily target", f"{daily_target:,.0f} cal", f"{daily_target - tdee:+,.0f} from maintenance")
d3.metric("Net daily deficit", f"{daily_fat_deficit - daily_muscle_surplus:,.0f} cal", f"= {daily_fat_deficit:,.0f} fat loss − {daily_muscle_surplus:,.0f} muscle build")

st.divider()

st.subheader("Check-in calendar")
start_date = st.date_input("Competition start date", value=date.today())
checkin_interval = st.selectbox("Check-in every", ["1 week", "2 weeks", "4 weeks"], index=1)
